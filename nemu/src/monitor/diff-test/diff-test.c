...
void difftest_step(uint32_t eip) {
  ...
  if(r.eip != cpu.eip){
      diff = true;
      printf("Diff: eip  QEMU: %08x\n",r.eip);
      printf("           NEMU: %08x\n",cpu.eip);
  }
  if(r.eax != cpu.eax){
      diff = true;
      printf("Diff: eax  QEMU: %08x\n",r.eax);
      printf("           NEMU: %08x\n",cpu.eax);
  }
  if(r.ecx != cpu.ecx){
      diff = true;
      printf("Diff: ecx  QEMU: %08x\n",r.ecx);
      printf("           NEMU: %08x\n",cpu.ecx);
  }
  if(r.ebx != cpu.ebx){
      diff = true;
      printf("Diff: ebx  QEMU: %08x\n",r.ebx);
      printf("           NEMU: %08x\n",cpu.ebx);
  }
  if(r.edx != cpu.edx){
      diff = true;
      printf("Diff: edx  QEMU: %08x\n",r.edx);
      printf("           NEMU: %08x\n",cpu.edx);
  }
  if(r.ebp != cpu.ebp){
      diff = true;
      printf("Diff: ebp  QEMU: %08x\n",r.ebp);
      printf("           NEMU: %08x\n",cpu.ebp);
  }
  if(r.esp != cpu.esp){
      diff = true;
      printf("Diff: esp  QEMU: %08x\n",r.esp);
      printf("           NEMU: %08x\n",cpu.esp);
  }
  if(r.esi != cpu.esi){
      diff = true;
      printf("Diff: esi  QEMU: %08x\n",r.esi);
      printf("           NEMU: %08x\n",cpu.esi);
  }
  if(r.edi != cpu.edi){
      diff = true;
      printf("Diff: edi  QEMU: %08x\n",r.edi);
      printf("           NEMU: %08x\n",cpu.edi);
  }
  if (diff) {
    nemu_state = NEMU_END;
